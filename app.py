...
class BMITransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        X['NewBMI'] = "Normal"
        X['BMI'] = pd.to_numeric(X['BMI'], errors='coerce')  # Convert BMI to numeric, setting errors to NaN
...
class GlucoseTransformer(BaseEstimator, TransformerMixin):
    def __init__(self):
        self.NewGlucose = pd.Series(["Low", "Normal", "Overweight", "Secret", "High"], dtype="category")
...
def handle_image_disease(request, disease):
    ...
    if file and allowed_file(file.filename):
        ...
        if disease == 'alzheimer':
            ...
            result = alzheimers_classes.get(result, 'Unknown')
